...
<section>
    <nav>
        <ul>
            <li><a href="/wrongAccess.jsp"><fmt:message key="REGISTER" bundle="${board}"/></a></li>
            <li><a href="/logout.do"><fmt:message key="LOGOUT" bundle="${board}"/></a></li>
            <li><a href="/write.do"><fmt:message key="WRITE" bundle="${board}"/></a></li>
        </ul>
    </nav>
    ...
    <article>
        ...
        <table>
            ...
            <% PostRepository postRepository = (PostRepository) application.getAttribute("postRepository");
                for (int i = 0; i < postRepository.getSize(); i++) {%>
            <tr>
                <td><%= postRepository.getPost(i).getTitle() %>
                </td>
                <td><%= postRepository.getPost(i).getWriterUserId() %>
                </td>
                <td>
                    <button class="button button1"
                            onclick="location.href='contentsShow.jsp?writer=<%= postRepository.getPost(i).getWriterUserId() %>'" name="id"
                    ><fmt:message key="SHOW"
                                  bundle="${board}"/></button>
                    <button class="button button2"
                            onclick="location.href='contentsModify.jsp?id=<%=session.getAttribute("id")%>&writer=<%= postRepository.getPost(i).getWriterUserId() %>'" name="id"
                    ><fmt:message
                            key="MODIFY"
                            bundle="${board}"/></button>
                    <button class="button button3" onclick="location.href='contentsDelete.jsp?id=<%=session.getAttribute("id")%>&writer=<%= postRepository.getPost(i).getWriterUserId() %>'"
                            name="id"
